...
(function () {
  ...
  async function manualAnalyze() {
    ...
    try {
      let websiteText = "";
      if (el("enableWebsiteCheck").checked && url) {
  ...
  async function runGeminiVision() {
    ...
    const enableWebsiteCheck = el("enableWebsiteCheck").checked;

    // Sheet 設定
    const enableWriteSheet = el("enableWriteSheet")?.checked;
...
- region：請只輸出「縣市層級」，例如「台中市 / 新竹縣 / 台北市 / 高雄市」。不要輸出完整地址、不要輸出國家。
